...
class BlandAltman:
    ...
    def plot_bland_altman(self, df, cnstar_threshold, png):
        ...
        for model_name, model_dict in self.dict_models.items():
            ...
            sns.scatterplot(
                data=data.loc[~age_mask, ], x='age', y=f'age_pred{model_dict["col_suffix"]}',
            ...
            ax.plot(xlim, ylim, linewidth=1, color=colors['diagonal'], alpha=alphas['diagonal'])
            ...
            corr = data.loc[age_mask, ['age', f'age_pred{model_dict["col_suffix"]}']].corr().iloc[0, 1]
            ax.text(0.5, 0.025, fr'$r = {corr:.3f}$', fontsize=fontsize, fontfamily=fontfamily, transform=ax.transAxes,
                    ha='center', va='bottom')
            ...
            ax.grid(visible=True, which='major', linewidth=0.5, color=colors['grid'], alpha=alphas['grid'])
...
if __name__ == '__main__':
    ...
    for t in cnstar_threshold_choices:
        bland_altman.plot_bland_altman(df, t, f'reports/figures/2025-01-15_Predicted_vs_Chronological_Age_selected_models_merged_combined_one_figure/figs/brain_age_vs_chronological_age_selected_models_cnstar-{t}.png')