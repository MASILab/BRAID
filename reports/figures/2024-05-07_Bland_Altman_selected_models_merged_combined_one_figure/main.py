...
def roster_brain_age_models():
    dict_models = {
        ...
        'GM age (DeepBrainNet)': {
            ...
            'yaxis_label': 'GM age (DeepBrainNet)\nbrain age gap (years)'
        },
        'GM age (TSAN)': {
...
class BlandAltman:
    ...
    def plot_bland_altman(self, df, cnstar_threshold, png):
        ...
        gs = gridspec.GridSpec(nrows=7, ncols=5, wspace=0, hspace=0, width_ratios=[0.3, 1, 1, 1, 1], height_ratios=[1, 1, 1, 1, 1, 0.2, 0.2])
        ...
        for model_name, model_dict in self.dict_models.items():
            ...
            sns.scatterplot(
                data=data.loc[age_mask&(data['dataset']=='ICBM'), ], x='age', y=f'age_pred{model_dict["col_suffix"]}_bag',
                s=marker_props['size'], linewidth=marker_props['linewidth'], color=colors['marker_second'], alpha=alphas['scatter_inside'],
                ax=ax
            )
            sns.scatterplot(
                data=data.loc[age_mask&(data['dataset']!='ICBM'), ], x='age', y=f'age_pred{model_dict["col_suffix"]}_bag',
            ...
            sns.kdeplot(
                data=data.loc[age_mask&(data['dataset']=='ICBM'), ], x='age', y=f'age_pred{model_dict["col_suffix"]}_bag',
                fill=True, levels=10, cut=1, cmap=colors['cmap_second'], alpha=alphas['kde'],
                ax=ax,
            )
            sns.kdeplot(
                data=data.loc[age_mask&(data['dataset']!='ICBM'), ], x='age', y=f'age_pred{model_dict["col_suffix"]}_bag',
        ...
        lg1 = mlines.Line2D([], [], color=colors['marker_second'], alpha=alphas['scatter_inside'], marker='o', linestyle='None', markersize=2)
        lg2 = mlines.Line2D([], [], color=colors['marker_main'], alpha=alphas['scatter_inside'], marker='o', linestyle='None', markersize=2)
        ax.legend(handles=[lg1, lg2], labels=['ICBM', 'other datasets'], 
                  loc='upper center', bbox_to_anchor=(0.5, 0.9), ncol=2, prop={'family': fontfamily, 'size':fontsize})
        ax.axis('off')
...
if __name__ == '__main__':
    ...
    for t in cnstar_threshold_choices:
        bland_altman.plot_bland_altman(df, t, f'reports/figures/2024-05-07_Bland_Altman_selected_models_merged_combined_one_figure/figs/brain_age_models_bland_altman_cnstar-{t}.png')