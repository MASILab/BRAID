""" Variant of v3 for testing.
Use the first instead of the last session of the MCI/dementia subjects.
"""
...
class BlandAltmanPlotWMGM:
            ...
            if 'normal' in rows_subj['diagnosis'].values:
                ...
                for i in range(len(rows_subj.index)-1):
                    if (rows_subj.iloc[i]['diagnosis'] == 'normal') & (rows_subj.iloc[i+1]['diagnosis'] in ['MCI', 'dementia']):
                        ...
                        break
        # the first session of the MCI subjects
        for subj in df.loc[df['diagnosis']=='MCI', 'dsubj'].unique():
            df.loc[(df['dsubj'] == subj) &
                   (df['age_gt'] == df.loc[(df['dsubj'] == subj)&(df['diagnosis']=='MCI'), 'age_gt'].min()), 'Category'] = 'First sample of MCI subjects'
        # the first session of the dementia subjects
        for subj in df.loc[df['diagnosis']=='dementia', 'dsubj'].unique():
            df.loc[(df['dsubj'] == subj) &
                   (df['age_gt'] == df.loc[(df['dsubj'] == subj)&(df['diagnosis']=='dementia'), 'age_gt'].min()), 'Category'] = 'First sample of dementia subjects'
    ...
    def make_bland_altman_plot(self, png, csv='reports/figures/2024-03-04_Bland_Altman_WM_vs_GM_Age/tmp.csv'):
        ...
        dict_brief_text = {
            ...
            'First sample of MCI subjects': 'MCI',
            'First sample of dementia subjects': 'dementia',
        }
...
if __name__ == '__main__':
    ...
    b.make_bland_altman_plot(png='reports/figures/2024-03-04_Bland_Altman_WM_vs_GM_Age/figs/bland_altman_wm_vs_gm_age_v3_first_session.png')